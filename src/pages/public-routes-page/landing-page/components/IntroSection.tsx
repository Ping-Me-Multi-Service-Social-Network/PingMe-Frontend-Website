...
function BentoFeaturesSection({ t }: Readonly<{ t: (key: string) => string }>) {
  ...
  // [H1 fix] Bỏ useSpring (tốn nhiều frame tính toán), dùng useTransform trực tiếp với CSS transition
  const { scrollYProgress } = useScroll({ target: ref, offset: ["start end", "end start"] });
  const yBg = useTransform(scrollYProgress, [0, 1], [40, -40]);

  // [H3 fix] Memoize features array — chỉ tạo lại khi ngôn ngữ thay đổi
  ...
  return (
    ...
    >
      {/* Subtle parallax blob — dùng useTransform trực tiếp, không cần useSpring */}
      <m.div
        style={{
          y: yBg,
          position: "absolute",
...
function BentoCard({
  ...
  return (
    <m.div
      ...
      whileHover={{ y: -4 }}
    >
      {/* [C3 fix] boxShadow hover chuyển sang CSS class, không dùng JS onMouseEnter/Leave */}
      <button
        type="button"
        onClick={onNavigate}
        className="bento-card h-full rounded-2xl overflow-hidden flex flex-col text-left"
        style={{
          width: "100%",
          padding: 0,
      ...
      >
        ...
        {!wide && (
          <div className={`relative ${imageHeight} overflow-hidden shrink-0`}>
            ...
            />
            {/* Tag chip — [H2 fix] bỏ backdropFilter trên tag chip, dùng solid background */}
            <span
              className="absolute top-4 left-4 px-3 py-1 rounded-full text-xs font-semibold tracking-wide"
              style={{ background: "rgba(255,255,255,0.95)", color: "oklch(0.12 0.03 292)" }}
            >
              {feature.tag}
            </span>
          </div>
